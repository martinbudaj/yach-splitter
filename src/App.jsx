...
export default function YachtSplitter() {
  ...
  const [modal, setModal] = useState(null); // { action: string, onConfirm: fn }
  ...
  useEffect(() => {
    const unsub = onValue(STATE_REF, (snap) => {
      const data = snap.val();
      if (data) {
  ...
  const removeCrew = (name) => {
    ...
    withPassword(`Odstrániť člena "${name}"`, () => {
      ...
      setCrew(newCrew); setExpenses(newExpenses); setSkipper(newSkipper);
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `yacht-splitter-${dateStr}.csv`; a.click();
  ...
  const importCSV = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        let newSkipper = skipper, newCrew = [];
        const newExpenses = [];
        lines.forEach(line => {
          if (line.startsWith("SKIPPER,")) newSkipper = line.split(",")[1].replace(/"/g, "").trim();
          else if (line.startsWith("CREW,")) newCrew = line.replace(/^CREW,"?/, "").replace(/"?\s*$/, "").split("|").map(s => s.trim()).filter(Boolean);
        ...
        if (newCrew.length >= 2) {
          withPassword("Importovať dáta zo súboru", () => {
            setCrew(newCrew); setSkipper(newSkipper); setExpenses(newExpenses);
            setForm(f => ({ ...f, payer: newCrew[0] }));
            saveState(newCrew, newSkipper, newExpenses);
            alert(`✅ Import úspešný: ${newCrew.length} členov, ${newExpenses.length} výdavkov.`);
          });
        } else alert("❌ Súbor neobsahuje platné zálohovacie dáta.");
      } catch { alert("❌ Chyba pri čítaní súboru."); }
    };
  ...
  const syncLabel = syncStatus === "ok" ? `Synced ${lastSync ? lastSync.toLocaleTimeString("sk-SK", { hour: "2-digit", minute: "2-digit" }) : ""}` : syncStatus === "error" ? "Chyba synchronizácie" : syncStatus === "saving" ? "Ukladám..." : "Načítavam...";
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #0a2540 0%, #0e3a5c 40%, #1a5276 100%)", fontFamily: "'Segoe UI', system-ui, sans-serif", color: "#e8f4f8" }}>
      ...
      <div style={{ textAlign: "center", padding: "36px 20px 8px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 10 }}>
          <button onClick={exportCSV} disabled={expenses.length === 0} style={{ background: expenses.length > 0 ? "rgba(46,204,113,0.2)" : "rgba(255,255,255,0.05)", border: `1px solid ${expenses.length > 0 ? "rgba(46,204,113,0.4)" : "rgba(255,255,255,0.1)"}`, borderRadius: 20, padding: "6px 16px", color: expenses.length > 0 ? "#2ecc71" : "#7fb9d8", fontSize: 13, fontWeight: 600, cursor: expenses.length > 0 ? "pointer" : "default" }}>
            ⬇ Export CSV
          </button>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "0 16px 80px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
          {[{ label: "Spolu výdavky", val: fmt(totalSpent), icon: "💰" }, { label: "Na osobu", val: fmt(perPerson), icon: "👤" }, { label: "Platcovia", val: payers.length + " ľudí", icon: "🧑‍✈️" }].map(s => (
        ...
        <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 16, padding: 16, marginBottom: 16, border: "1px solid rgba(255,255,255,0.1)" }}>
              ...
              return (
                <div key={c} style={{ background: isSk ? "rgba(241,196,15,0.2)" : "rgba(127,185,216,0.2)", border: `1px solid ${isSk ? "rgba(241,196,15,0.4)" : "rgba(127,185,216,0.3)"}`, borderRadius: 20, padding: "4px 10px 4px 8px", fontSize: 13, display: "flex", alignItems: "center", gap: 5 }}>
                  <span onClick={() => handleSetSkipper(c)} style={{ cursor: "pointer", userSelect: "none" }}>
                    {isSk ? "⚓" : "👤"} {c}{isSk && <span style={{ fontSize: 10, color: "#f1c40f", marginLeft: 3, fontWeight: 700 }}>SKIPER</span>}
                  </span>
                  {crew.length > 2 && <button onClick={() => removeCrew(c)} style={{ background: "none", border: "none", color: "#7fb9d8", cursor: "pointer", fontSize: 14, padding: 0 }}>×</button>}
                </div>
              );
            })}
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input value={newCrewName} onChange={e => setNewCrewName(e.target.value)} onKeyDown={e => e.key === "Enter" && addCrew()} placeholder="Pridať člena..." style={{ flex: 1, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, padding: "8px 12px", color: "#fff", fontSize: 13, outline: "none" }} />
            ...
            <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 16, padding: 16, marginBottom: 16, border: "1px solid rgba(255,255,255,0.1)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <input type="number" min="0" step="0.01" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} placeholder="0.00" style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, padding: "9px 10px", color: "#fff", fontSize: 13, outline: "none" }} />
                </div>
              </div>
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 11, color: "#7fb9d8", marginBottom: 4 }}>Popis</div>
                <input value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} onKeyDown={e => e.key === "Enter" && addExpense()} placeholder="napr. Tankování, potraviny, kotviště..." style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, padding: "9px 12px", color: "#fff", fontSize: 13, outline: "none" }} />
              </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 16, padding: 16, marginBottom: 16, border: "1px solid rgba(255,255,255,0.1)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {crew.map(c => {
                  ...
                  const isPos = bal > 0.005, isNeg = bal < -0.005, isSkip = c === skipper;
                  return (
                    <div key={c} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 32, height: 32, borderRadius: "50%", background: isSkip ? "rgba(241,196,15,0.25)" : isPos ? "rgba(46,204,113,0.25)" : isNeg ? "rgba(231,76,60,0.25)" : "rgba(127,185,216,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: isSkip ? "#f1c40f" : isPos ? "#2ecc71" : isNeg ? "#e74c3c" : "#7fb9d8" }}>{isSkip ? "⚓" : c[0]}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: "#fff" }}>{c} {isSkip && <span style={{ fontSize: 10, color: "#f1c40f", fontWeight: 700 }}>SKIPER</span>}</div>
                        <div style={{ fontSize: 11, color: "#7fb9d8" }}>zaplatil {fmt(expenses.filter(e => e.payer === c).reduce((s, e) => s + e.amount, 0))}{isSkip ? " · podiel 0 €" : ` · podiel ${fmt(perPerson)}`}</div>
                      </div>
                      <div style={{ fontSize: 14, fontWeight: 800, color: isSkip ? "#f1c40f" : isPos ? "#2ecc71" : isNeg ? "#e74c3c" : "#7fb9d8" }}>{(isPos || (isSkip && bal > 0)) ? "+" : ""}{fmt(bal)}</div>
                    </div>
                  );
                })}
              </div>
